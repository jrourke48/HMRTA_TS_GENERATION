...
## Repository Breakdown
 - **Transition_Systems**: Preliminary implementations of state-space abstractions for workspace discretization. Contains multiple TS variants (generic and grid-world based) exploring state representation models and discretization strategies.
 - **Automatons**: Polymorphic graph-based automata framework implementing node-edge hierarchies. Base Automaton class with three specialized subclasses: BüchiAutomaton (Encodes Task Specification), ProductAutomaton (synchronous product), and TransitionSystem (workspace discretization). Core synthesis component of MCTB-PDT (Multi-Capability Task Batch Planning Decision Tree).
- **MCTB-PDT (Multi-Capability Task Batch Planning Decision Tree)**: This is the main contribution of the thesis project. Solving for the product automaton which combines the task specification applied to your environment (Transition System). This can be extremely cumbersome and the state space explodes here, especially as the number of robots, transition system states, and NBA states increase. Our approach uses a Task Batch
Planning Decision Tree (TB-PDT) is presented in this section,
which encodes the automaton states generated by φ and the
system sates in a hierarchical tree structure. By growing the tree
from the root node to leaf nodes, TB-PDT can be searched for
satisfactory task allocations. this tree approach allows us to find a correct solution while only searching a small fraction of the product.

### Key Features n

- **LTL-based Task Specification**: Define complex task requirements using Linear Temporal Logic formulas
- **Standard Büchi Automata (SBA)**: Automatic conversion of LTL formulas to accepting automata
